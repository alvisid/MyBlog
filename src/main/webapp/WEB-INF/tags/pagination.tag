...
<%@ attribute name="pagination" required="true" type="net.devstudy.blog.model.Pagination" %>
...
<c:if test="${pagination != null}">
    <ul class="pagination text-center" role="navigation" aria-label="Pagination">
        ...
        <li class="pagination-next${pagination.next ? '' : ' disabled' }">
            <c:choose>
                <c:when test="${pagination.next}"><a href="${pagination.nextUrl}"
                                                     aria-label="Next page">Next</a></c:when>
